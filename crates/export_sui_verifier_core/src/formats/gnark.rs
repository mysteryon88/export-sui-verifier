...
fn validate_gnark_ic_len(ic_len: usize, remaining: usize, minimum_point_size: usize) -> Result<()> {
    let max_for_sui = crate::model::MAX_PUBLIC_INPUTS + 1;
    let max_for_bytes = remaining / minimum_point_size;
    if ic_len == 0 || ic_len > max_for_sui || ic_len > max_for_bytes {
        return Err(Error::IcLengthMismatch(format!(
            "gnark G1.K length {ic_len} exceeds Sui/remaining-byte limits"
        )));
    }
    Ok(())
}
...
mod tests {
    ...
    fn rejects_gnark_ic_lengths_before_untrusted_allocation() {
        ...
        assert!(validate_gnark_ic_len(10, 320, 32).is_err());
        assert!(validate_gnark_ic_len(9, 288, 32).is_ok());
    }

    #[test]
    fn rejects_oversized_gnark_binary_before_reading_it() {
        let path = std::env::temp_dir().join(format!(
            "export_sui_verifier_oversized_gnark_{}",
            std::process::id()
        ));
